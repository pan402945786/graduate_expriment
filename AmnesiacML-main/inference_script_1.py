...
#     in_loader = torch.utils.data.DataLoader(shadow_datasets[0][0], batch_size=batch_size, shuffle=True)
...
# exit("shadow model train finished")
...
# for c in range(100):
    ...
    for i, shadow_model_set in enumerate(shadow_models):
        ...
        shadow_model = shadow_model_set[i]
        # shadow_model = shadow_model_set[i].to(device)
        path = f"infattack/resnet-shadow_model_{i}.pt"

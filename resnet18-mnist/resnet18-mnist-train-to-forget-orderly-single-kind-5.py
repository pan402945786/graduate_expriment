...
# checkpoint = torch.load(r"D:\ww2\graduate_expriment\resnet18-vggface100-2\model\resnet18_vggface100_reset_13_before_training.pth", map_location='cpu')
...
paramList, freezeParamList = generateParamsResnet18(net, finishedModel, initModel, layeredParams, False, filePath,
                                                    strucName, datasetName, [3])
# print(freezeParamList)
# print('exit')
# exit()

# 准备数据集并预处理
for forgetItem in range(10):
    forget = [forgetItem]
    ...
    forgetTestLoader = DataLoader(test_set_forget_set, batch_size=64, shuffle=True)
    for paramIndex, param in enumerate(paramList):
        ...
        forgetItemName = "_forgetitem_"+str(forgetItem)
        checkpoint = torch.load(fileName)
        net.load_state_dict(checkpoint)
        fileAccName = fileName +forgetItemName+ "_after_acc.txt"
        fileLogName = fileName + forgetItemName+"_after_log.txt"
        fileModelName = fileName + forgetItemName+"_after_training"
        # 冻结相关层
        frozenIndex = []
        paramCount = 0
        ...
        with open(fileAccName, "a+") as f:
            with open(fileLogName, "a+")as f2:
                for epoch in range(pre_epoch, EPOCH):
                    print("current forget item:" + str(forgetItem))
                    print('\nEpoch: %d' % (epoch + 1))
                    net.train()
                    sum_loss = 0.0
                    correct = 0.0
                    ...
                    with torch.no_grad():
                        ...
                        if acc > best_acc:
                            ...
                            torch.save(net.state_dict(), '%s/%s_best_acc_model.pth' % (
                                args.outf, param+forgetItemName))
                    ...
                    if lastTrainLoss < T_threshold and epoch > tolerate:
                        ...
                        torch.save(net.state_dict(),
                                   '%s/%s_%03d_epoch.pth' % (args.outf, param.replace("before", "after")+forgetItemName, epoch + 1))
                        f.write("train loss达到限值%s，提前退出" % lastTrainLoss)
                        f.write('\n')
                        f.flush()
                        break
                    if optimizer.state_dict()['param_groups'][0]['lr'] < LR_threshold:
                        print("学习率过小，退出")
                        f.write("学习率过小，退出")
                        f.write('\n')
                        f.flush()

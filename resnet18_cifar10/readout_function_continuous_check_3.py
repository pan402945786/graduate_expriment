...
for forget_num in range(1, 10):
    ...
    with torch.no_grad():
        ...
        for data in testloader_forget:
            ...
            checkpoint = torch.load("./model/" + targetFiles[forget_num-1])
            net.load_state_dict(checkpoint)
            outputs = net(images)
            prbblt_target = np.array(torch.nn.functional.softmax(outputs).cpu())
            checkpoint = torch.load("./model/" + savedFiles[forget_num-1])
            net.load_state_dict(checkpoint)
            outputs = net(images)
            prbblt_pred = np.array(torch.nn.functional.softmax(outputs).cpu())
            norm_1s += cal_norm(prbblt_pred, prbblt_target, 1)
            norm_2s += cal_norm(prbblt_pred, prbblt_target, 2)
        print(
            savedFiles[forget_num - 1] + '与目标文件' + targetFiles[forget_num - 1] + '在遗忘集上的第一范数距离为%.5f，第二范数距离为%.8f' % (
            1. * norm_1s / total_count, 1. * norm_2s / total_count))
        ...
        for data in testloader_unforget:
            ...
            norm_2s += cal_norm(prbblt_pred, prbblt_target, 2)
        print(
            savedFiles[forget_num - 1] + '与目标文件' + targetFiles[forget_num - 1] + '在保留集上的第一范数距离为%.5f，第二范数距离为%.8f' % (
            1. * norm_1s / total_count, 1. * norm_2s / total_count))
    ...
    #     print(savedFiles[forget_num-1] + '测试保留集分类准确率为：%.3f%%' % (100. * corrects / totals))
    #
    # totals = 0.
    # corrects = 0.
    # with torch.no_grad():
    ...
    #         totals += labels.size(0)
    #         corrects += (predicted == labels).sum()
    #     print(savedFiles[forget_num-1] + '测试遗忘集分类准确率为：%.3f%%' % (100. * corrects / totals))
    # print("---------------------------------------------------------------")
    # totals = 0.
    # corrects = 0.
    # with torch.no_grad():
    ...
    #         totals += labels.size(0)
    #         corrects += (predicted == labels).sum()
    #     print(targetFiles[forget_num-1] + '测试保留集分类准确率为：%.3f%%' % (100. * corrects / totals))
    # totals = 0.
    # corrects = 0.
    # with torch.no_grad():
    #     for data in testloader_forget:
    ...
    #         totals += labels.size(0)
    #         corrects += (predicted == labels).sum()
    #     print(targetFiles[forget_num-1] + '测试遗忘集分类准确率为：%.3f%%' % (100. * corrects / totals))
    # print("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")

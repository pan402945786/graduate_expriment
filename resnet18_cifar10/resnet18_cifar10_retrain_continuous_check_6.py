...
for forget_num in range(1, 10):
    ...
    if __name__ == "__main__":
        ...
        scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=3, verbose=True,
                                      ...
                                      eps=1e-08)
        # 网络参数初始化
        checkpoint = torch.load(args.outf + "resnet18_cifar10_forget_two_kinds_init.pth", map_location='cpu')
        net.load_state_dict(checkpoint)
        print('load files:resnet18_cifar10_forget_two_kinds_init.pth')

        with open("resnet18_cifar10_retrain_acc_continuous_check_forget_"+str(forget_num)+"_kinds.txt", "a+") as f:
            with open("resnet18_cifar10_retrain_log_continuous_check_forget_"+str(forget_num)+"_kinds.txt", "a+")as f2:
                for epoch in range(pre_epoch+1, EPOCH+1):
                    ...
                    with torch.no_grad():
                        ...
                        if epoch % 10 < 1 and pre_epoch != epoch:
                            print('Saving model......')
                            torch.save(net.state_dict(), args.outf + '/resnet18_cifar10_retrain_' + str(epoch) + '_continuous_check_forget_'+str(forget_num)+'_kinds.pth')
                ...
                torch.save(net.state_dict(),
                           args.outf + '/resnet18_cifar10_retrain_' + str(epoch) + '_continuous_check_forget_'+str(forget_num)+'_kinds.pth')

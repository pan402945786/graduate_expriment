...
parser.add_argument('--end_num', type=int, default=14, help='end number to forget')
...
for forget_num in range(1, 10):
    ...
    for i, item in enumerate(layer_count_list):
        frozenLayer = []
        for j in range(14-item):
            frozenLayer = frozenLayer + layeredParams[j]
        preparedFrozenLayers.append(frozenLayer)

    savedFiles = [
        'vgg16_cifar10_reset_fc_before_training.pth',
        'vgg16_cifar10_reset_fc_conv1_before_training.pth',
        'vgg16_cifar10_reset_fc_conv2_before_training.pth',
        'vgg16_cifar10_reset_fc_conv3_before_training.pth',
        'vgg16_cifar10_reset_fc_conv4_before_training.pth',
        'vgg16_cifar10_reset_fc_conv5_before_training.pth',
        'vgg16_cifar10_reset_fc_conv6_before_training.pth',
        'vgg16_cifar10_reset_fc_conv7_before_training.pth',
        'vgg16_cifar10_reset_fc_conv8_before_training.pth',
        'vgg16_cifar10_reset_fc_conv9_before_training.pth',
        'vgg16_cifar10_reset_fc_conv10_before_training.pth',
        'vgg16_cifar10_reset_fc_conv11_before_training.pth',
        'vgg16_cifar10_reset_fc_conv12_before_training.pth',
        'vgg16_cifar10_reset_fc_conv13_before_training.pth',
    ]
    ...
    if __name__ == "__main__":
        ...
        for i, item in enumerate(layer_count_list):
            ...
            for param in net.parameters():
                ...
                j = j + 1
            with open(savedFiles[item-1] + "_acc_continuous_check_forget_"+str(forget_num)+"_kinds.txt", "a+") as f:
                with open(savedFiles[item-1] + "_log_continuous_check_forget_"+str(forget_num)+"_kinds.txt", "a+")as f2:
                    for epoch in range(pre_epoch+1, EPOCH+1):
                        ...
                        for k, data in enumerate(trainloader, 0):
                            ...
                            print('[forgetNum:%d, epoch:%d, iter:%d] Loss: %.03f | Acc: %.3f%% | Time: %s | File: %s | LR: %.6f'
                                  % (forget_num, epoch, (k + 1 + (epoch-1) * length), sum_loss / (k + 1), 100. * correct / total, time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), savedFiles[item-1], optimizer.state_dict()['param_groups'][0]['lr']))
                        ...
                        with torch.no_grad():
                            ...
                            if epoch % 10 < 1 and pre_epoch != epoch:
                                print('Saving model......')
                                torch.save(net.state_dict(), args.outf + '/' + savedFiles[item - 1] + '_finetuning_' + str(epoch) + '_continuous_check_forget_'+str(forget_num)+'_kinds.pth')
                    ...
                    torch.save(net.state_dict(),
                               args.outf + '/' + savedFiles[item - 1] + '_forget_two_kinds_after_finetuning_' + str(epoch) + '_continuous_check_forget_'+str(forget_num)+'_kinds.pth')

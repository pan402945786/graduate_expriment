        ...
        with open(savedFiles[item-1] + "_acc.txt", "a+") as f:
            with open(savedFiles[item-1] + "_log.txt", "a+")as f2:
                for epoch in range(pre_epoch+1, EPOCH+1):
                    ...
                    with torch.no_grad():
                        ...
                        if epoch % 20 < 1 and pre_epoch != epoch:
                            print('Saving model......')
                            torch.save(net.state_dict(), args.outf + '/' + savedFiles[item - 1] + '_finetuning_' + str(epoch) + '_epochs.pth')
                ...
                torch.save(net.state_dict(),
                           args.outf + '/' + savedFiles[item - 1] + '_forget_two_kinds_after_finetuning_' + str(EPOCH) + '_epochs.pth')

...
if __name__ == "__main__":
    ...
    scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=3, verbose=True,
                                  ...
                                  eps=1e-08)
    # 网络参数初始化
    # torch.save(net.state_dict(), args.outf + '/vgg16_cifar10_normal_train_init.pth')
    checkpoint = torch.load(args.outf + "vgg16_cifar10_normal_tarin_init.pth", map_location='cpu')
    net.load_state_dict(checkpoint)
    print('load files:vgg16_cifar10_normal_tarin_init.pth')

    with open("vgg16_cifar10_normal_tarin_acc.txt", "a+") as f:
        with open("vgg16_cifar10_normal_train_log.txt", "a+")as f2:
            for epoch in range(pre_epoch+1, EPOCH+1):
                ...
                for k, data in enumerate(trainloader, 0):
                    ...
                    print('[epoch:%d, iter:%d] Loss: %.03f | Acc: %.3f%% | Time: %s | LR: %.6f'
                    ...
                    f2.write('[epoch:%d, iter:%d] |Loss: %.03f | Acc: %.3f%% | Time: %s | LR: %.6f'
                          % (epoch, (k + 1 + (epoch-1) * length), sum_loss / (k + 1), 100. * correct / total, time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), optimizer.state_dict()['param_groups'][0]['lr']))
                    f2.write('\n')
                    f2.flush()
                ...
                with torch.no_grad():
                    ...
                    if epoch % 10 < 1 and pre_epoch != epoch:
                        print('Saving model......')
                        torch.save(net.state_dict(), args.outf + '/vgg16_cifar10_normal_train_'+ str(epoch) +'_epochs.pth')
            ...
            torch.save(net.state_dict(),
                       args.outf + '/vgg16_cifar10_normal_train_finish_'+str(EPOCH)+'_epochs.pth')

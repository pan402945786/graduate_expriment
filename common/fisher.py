...
trainset = torchvision.datasets.CIFAR10(root='D:\ww2\graduate_expriment\data', train=True, download=False, transform=transform_train) #训练数据集
testset = torchvision.datasets.CIFAR10(root='D:\ww2\graduate_expriment\data', train=False, download=False, transform=transform_test)
print(len(trainset))
print(len(testset))
...
train_loader_full, valid_loader_full, test_loader_full = datasets.get_loaders(dataset, batch_size=args.batch_size,
                                                                              seed=seed, augment=False, shuffle=True)
marked_loader, _, _ = datasets.get_loaders(dataset, class_to_replace=class_to_forget,
                                           num_indexes_to_replace=num_to_forget, only_mark=True, batch_size=1,
                                           seed=seed, augment=False, shuffle=True)

...
# print('Saving model......')
# torch.save(net.state_dict(), '%s/net_train_8_kinds_init.pth' % args.outf)

checkpoint = torch.load("./model/net_train_8_kinds_init.pth", map_location='cpu')
...
if __name__ == "__main__":
    ...
    with open("normal_train_8_kinds_acc.txt", "w") as f:
        with open("normal_train_8_kinds_log.txt", "w")as f2:
            for epoch in range(pre_epoch, EPOCH):
                ...
                with torch.no_grad():
                    ...
                    if epoch == 20 or epoch == 40 or epoch == 80:
                        print('Saving model......')
                        torch.save(net.state_dict(), '%s/net_normal_train_8_kinds_%03d_epoch.pth' % (args.outf, epoch + 1))
                    f.write("EPOCH=%03d,Accuracy= %.3f,Time:%s%%" % (epoch + 1, acc, time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())))
                    f.write('\n')
                    f.flush()
                    # 记录最佳测试分类准确率并写入best_acc.txt文件中
                    if acc > best_acc:
                        f3 = open("train_8_kinds_best_acc.txt", "w")
                        f3.write("EPOCH=%d,best_acc= %.3f%%" % (epoch + 1, acc))
                        f3.close()
                        best_acc = acc
            print('Saving model......')
            torch.save(net.state_dict(), '%s/net_normal_train_8_kinds_%03d.pth' % (args.outf, epoch + 1))
